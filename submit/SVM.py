def train_SVM():
    ...
    train_img = train['tr_images']
    #plt.imshow(np.swapaxes(np.reshape(train_img[0], (y, x)), 0, 1), cmap=pylab.gray())
    #plt.show()

    clf = svm.SVC(kernel='poly', degree=2)
    # clf = svm.SVC(kernel='linear', C=10)
    clf.fit(train_img, np.reshape(train_original["tr_labels"], (n_images, )))
    # scores = cross_validation.cross_val_score(clf, train_img, np.reshape(train_original["tr_labels"], (n_images, )) , cv=3)
